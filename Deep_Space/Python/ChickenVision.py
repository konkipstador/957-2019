...
def findTargets(contours, image, centerX, centerY):
    ...
    # Puts the first 4 contour locations in NetworkTables.
    if len(contourX) > 0:
        table.putNumber("c1", contourX[0])
    else:
        table.putNumber("c1", 0)
  
    if len(contourX) > 1:
        table.putNumber("c2", contourX[1])
    else:
        table.putNumber("c2", 0)

    if len(contourX) > 2:
        table.putNumber("c3", contourX[2])
    else:
        table.putNumber("c3", 0)

    if len(contourX) > 3:
        table.putNumber("c4", contourX[3])
    else:
        table.putNumber("c4", 0)
...
if __name__ == "__main__":
    ...
    cameraServer1 = streams[0]
    # Get a CvSink. This will capture images from the camera
    cvSink1 = cameraServer1.getVideo()

    # (optional) Setup a CvSource. This will send images back to the Dashboard
    outputStream = cameraServer1.putVideo("stream", image_width, image_height)
    ...
    while True:
        ...
        timestamp1, img1 = cvSink1.grabFrame(img1)

        if (timestamp1 == 0):
            continue

        #threshold1 = threshold_video(img1)
        #processed1 = findContours(img1, threshold1)

        sd.putNumber("Framerate", 1/(time.time() - frameTime))
